...
const extractDetailsFromMessage = async (messageText, session) => {
    try {
        // Pass the message to OpenAI to extract structured data (like quantity, product, location)
        const response = await getOpenAIResponse(messageText);

        // Parse the response for details (you can adapt this part to fit your needs)
        const extractedDetails = parseOpenAIResponse(response);

        // Check for missing details and update the session
        if (extractedDetails.quantity) {
            session.data.quantity = extractedDetails.quantity;
        }
        if (extractedDetails.location) {
            session.data.location = extractedDetails.location;
        }
        if (extractedDetails.product) {
            session.data.product = extractedDetails.product;
        }

        // Check for missing data and prompt for further information
        if (!session.data.city) {
            await sendToWhatsApp(from, "❓ It seems like you're missing the city. Can you please provide the city?");
        }

        if (!session.data.quantity) {
            await sendToWhatsApp(from, "❓ Can you please specify the quantity (in liters)?");
        }

    } catch (error) {
        console.error('❌ Error extracting details from message:', error);
